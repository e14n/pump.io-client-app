// auth.js
//
// AuthC and AuthZ middleware for pump.io-enabled apps
//
...
var userAuth = function(req, res, next) {
    ...
    if (req.session.userID) {
        req.log.debug({userID: req.session.userID}, "Logging in with session-stored user ID");
        User.get(req.session.userID, function(err, user) {
            if (err) {
                next(err);
            } else {
                req.log.debug({userID: user.id}, "Logged in");
                req.user = user;
        ...
        req.log.debug({rememberme: req.cookies.rememberme}, "Logging in with rememberme cookie");
        async.waterfall([
            function(callback) {
            ...
            function(rm, callback) {
                var id = rm.user;
                req.log.debug({rememberme: req.cookies.rememberme, userID: id}, "Found rememberme cookie");
                async.parallel([
                    function(callback) {
                        rm.del(callback);
                    },
            ...
            if (err && err.name == "NoSuchThingError") {
                ...
                req.log.debug({userID: req.user.id}, "Set user");

                res.cookie("rememberme", rm.uuid, {path: "/", expires: new Date(Date.now() + 180 * 24 * 60 * 60 * 1000), httpOnly: true});
                req.log.debug({rememberme: rm.uuid, userID: req.user.id}, "Set rememberme cookie");
